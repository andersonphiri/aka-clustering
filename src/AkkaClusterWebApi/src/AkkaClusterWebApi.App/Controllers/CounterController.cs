...
using AkkaClusterWebApi.Domain;
...
public class CounterController : ControllerBase
{
    ...
    {
        var result = await _counterActor.Ask<CounterCommandResponse>(new IncrementCounterCommand(counterId, increment),
    ...
    {
        var result = await _counterActor.Ask<CounterCommandResponse>(new SetCounterCommand(counterId, counterValue),
            TimeSpan.FromSeconds(5));
        if (!result.IsSuccess)
        {
            return BadRequest();
